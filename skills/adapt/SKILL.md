...
### Pattern C: Sync Script (Canonical Source + Generation)

A single canonical source (typically `.claude/skills/`) is the source of truth. Sync scripts use `sed` or similar text replacement to generate platform-specific copies. This avoids manual triplication while supporting per-platform differences.

```text
project/
├── .claude/skills/my-skill/SKILL.md    # Canonical source
├── .agents/skills/my-skill/SKILL.md    # Generated by sync-codex.sh
├── .opencode/skills/my-skill/SKILL.md  # Generated by sync-opencode.sh
├── scripts/
│   ├── sync-codex.sh                   # sed replacements for Codex
│   └── sync-opencode.sh               # sed replacements for OpenCode
└── README.md
```

**What sync scripts replace:**

| Element | Claude Code (canonical) | Codex | OpenCode |
|---------|------------------------|-------|----------|
| Interactive tool | `AskUserQuestion` | Direct prompting | `question` |
| Skill invocation | `/plugin:skill` | `$plugin skill` | `/plugin_skill` |
| Subagent | `Agent tool` | `Codex subagent` | `@mention` |
| Skill directory | `.claude/skills/` | `.agents/skills/` | `.opencode/skills/` |

**When to use**: The project has skills that need minor per-platform syntax differences (tool names, invocation format, subagent syntax). The core content is identical but surface syntax differs.

**Real-world examples**: autoresearch (canonical `.claude/` + `sync-opencode.sh` + `sync-codex.sh`).

**Key rule**: Run sync scripts in CI to ensure generated files never drift from the canonical source. Add a CI check: `bash scripts/sync-codex.sh && git diff --exit-code .agents/`.
...
| Do skills need per-platform syntax tweaks (tool names, invocation format)? | **C: Sync Script** | **B: Adapters** (default for complex projects) |
